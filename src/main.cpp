...
void initShaders(GLuint * program) {
  ...
  void mainLoop() {
    ...
    while (!glfwWindowShouldClose(window)) {
      ...
      // Billboarded base
      GLint viewDirLoc = glGetUniformLocation(program[PROG_BOID], "u_viewDir");
      glm::vec3 viewDir = glm::normalize(lookAt - cameraPosition);
      glUniform3fv(viewDirLoc, 1, &viewDir[0]);
  ...
  void mousePositionCallback(GLFWwindow* window, double xpos, double ypos) {
    if (leftMousePressed) {
      ...
      zoom = std::fmax(0.1f, std::fmin(zoom, 20.0f));
      updateCamera();
    }
